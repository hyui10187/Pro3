...
public class RegisterAccount : LoginBase {
    ...
    private void CustomSignUp() { // 회원가입을 해주는 메소드
        ...
        // 회원가입을 할때는 3개의 파라미터를 던져줘야 한다  // 1: ID, 2: PW, 3: 콜백함수
        Backend.BMember.CustomSignUp(idInputField.text, pwInputField.text, callback => {
            ...
            if(callback.IsSuccess()) { // 계정생성 성공       // 계정 생성에 성공하면 계정의 이메일 정보도 업데이트 해주기
                Backend.BMember.UpdateCustomEmail(emailInputField.text, callbackParam => {
                    if(callbackParam.IsSuccess()) { // 이메일을 업데이트 하는 것에 성공했으면
                        SetMessage($"회원가입이 완료되었습니다. {idInputField.text}님 환영합니다.");
                        BackendGameData.Instance.GameDataInsert(); // 회원가입을 하면 기본 데이터를 서버의 DB에 저장해주기
                        ChangeSceneManager.LoadScene(SceneNames.LobbyScene); // 회원가입이 완료되면 바로 로그인을 시켜줘서 로비씬으로 이동시켜주기
                    }
                });
            } else { // 계정생성 실패
